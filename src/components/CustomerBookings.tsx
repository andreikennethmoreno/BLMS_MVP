...
const CustomerBookings: React.FC = () => {
  ...
  const filteredBookings = customerBookings.filter((booking: Booking) => {
    ...
    if (statusFilter === "current")
      return isCurrent(booking.checkIn, booking.checkOut);
  ...
  const stats = {
    ...
    upcoming: customerBookings.filter((b: Booking) => isUpcoming(b.checkIn))
      .length,
    current: customerBookings.filter((b: Booking) =>
      isCurrent(b.checkIn, b.checkOut)
    ).length,
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            ${Math.round(booking.totalAmount / nights)} per
                            night
                          </p>
                        </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex items-start space-x-4">
                    <img
                      src={
                        getProperty(selectedBooking.propertyId)?.images[0] || ""
                      }
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-2">
                        Host:{" "}
                        {getOwner(
                          getProperty(selectedBooking.propertyId)?.ownerId ?? ""
                        )?.name || "Unknown"}
                      </p>
                      {getProperty(selectedBooking.propertyId)
                        ?.maxStayDisplay && (
                        <div className="mt-2">
                          <p className="text-sm text-gray-600">
                      ...
                      <div className="mt-3">
                        <span
                          className={`inline-flex items-center space-x-1 px-3 py-1 rounded-lg text-sm font-medium ${getStatusColor(
